...
export async function createComponentType(compendium: string, folder: string, components: Array<Lattice|Mold|Impurity>): Promise<Array<EquipmentPF2e>> {
    let items = [];
    for (const component of components) {
        ...
        items.push(item as EquipmentPF2e);
    }
    if (!items) {
